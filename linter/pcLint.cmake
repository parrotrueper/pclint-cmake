...
# architecture
set(PCFG_ARCH_ARM "--compiler-options=-m32")
set(PCFG_ARCH_HOST "")
# output files
set(COMP_H "${CMAKE_BINARY_DIR}/co-gcc.h")
set(COMP_LNT "${CMAKE_BINARY_DIR}/co-gcc.lnt")
set(PCFG_LNTOUT "--config-output-lnt-file=${COMP_LNT}")
set(PCFG_HOUT "--config-output-header-file=${COMP_H}")


# pc lint helper
# armhost      - parameter for our compiler config script, valid values:
#                  * arm - for elves
#                  * host - for tests
# sources      - source files to lint
# include_dirs - header dirs to lint
# defines      - #defines for the project
# lib_dirs     - includes not to lint
# result       - html result file
function (add_pc_lint armhost sources include_dirs defines lib_dirs result)
    ...
    file(WRITE "${CMAKE_BINARY_DIR}/project.lnt" ${__filedata})
    file(TO_NATIVE_PATH "${CMAKE_BINARY_DIR}/project.lnt" __target_settings_file)
    ...
    # project settings
    set(PROJ_LNT "${CMAKE_BINARY_DIR}/project.lnt")

    set(TOHTML "${LH_DIR}/${PCL_GREP}")
    set(OUT_FILE "${CMAKE_BINARY_DIR}/lint_results.html")
    ...
    add_custom_target(pclint
        ...
        # pc lint
        COMMAND ${PCLINT} -os[${CMAKE_BINARY_DIR}/lint_results.xml] ${XML_LNT} ${OPT_LNT} ${MISRA_LNT} ${COMP_LNT} ${PROJ_LNT}
        # format to html
        COMMAND python3 ${TOHTML} --input-xml "${CMAKE_BINARY_DIR}/lint_results.xml" --output-html ${result}
    )
